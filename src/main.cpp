...
void render() {
    ...
    if (is_game == true && is_help == false) {
        ...
        // for (int i = 0; i < 4; ++i) {
        //     tile.set_location(vec3(1 / TILE_SCALE * i, -20, -40));
        //     tile.render(program);
        // }
        army.render(program);
        // Obtacle
...
bool user_init() {
    ...
    spike.set_direction(vec3(0, 0, 0));
    spike.set_angle(0);
    spike.set_location(vec3(0, -10, -20));

    tile.init("../bin/mesh/tile/tile.obj", "../bin/mesh/tile/tile.png");
    tile.set_scale(TILE_SCALE);

    army.init("../bin/mesh/army_big/army_big.obj", nullptr);
    army.set_scale(0.04f);
    army.set_direction(vec3(0, 1, 0));
    army.set_angle(-PI / 2);
    army.set_location(vec3(50, -10, -40));

    // load the mesh
    pMesh_player = load_model(player_obj);
    if (pMesh_player == nullptr) {
        printf("Unable to load mesh\n");
        return false;
    }
    pMesh_thorn = load_model(thorn_obj);
    if (pMesh_thorn == nullptr) {
        printf("Unable to load mesh\n");
        return false;
    }
    pMesh_cube = load_model(cube_obj);
    if (pMesh_cube == nullptr) {
        printf("Unable to load mesh\n");
        return false;
    }
    pMesh_enemy = load_model(enemy_obj);
    if (pMesh_enemy == nullptr) {
        printf("Unable to load mesh\n");
        return false;
    }
    pMesh_goal = load_model(goal_obj);
    if (pMesh_goal == nullptr) {
        printf("Unable to load mesh\n");
        return false;
    }
